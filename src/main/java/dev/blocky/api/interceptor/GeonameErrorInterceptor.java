...
public class GeonameErrorInterceptor implements Interceptor
{
    @Override
    public Response intercept(@NonNull Chain chain) throws IOException
    {
        ...
        if (!response.isSuccessful())
        {
            ...
            int value = status.getInt("value");

            switch (response.code())
            {
                case 400 -> throw new BadRequest(message);
                case 401 -> throw new Unauthorized(message);
                case 500 -> throw new InternalServerException(STR."Internal Server Error: \{value}, \{message}");
                default -> throw new HTTPException(STR."\{value}, \{message}");
            }
        }
        return response;
    }
