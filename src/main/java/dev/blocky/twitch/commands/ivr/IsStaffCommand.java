...
public class IsStaffCommand implements ICommand
{
    ...
    public void onCommand(@NonNull ChannelMessageEvent event, @NonNull TwitchClient client, @NonNull String[] prefixedMessageParts, @NonNull String[] messageParts) throws Exception
    {
        ...
        if (!ivrfi.getRoles().get("isStaff").asBoolean())
        {
            chat.sendMessage(event.getChannel().getName(), STR."Saved \{ivrfi.getDisplayName()} is not a Twitch staff at the moment.");
            return;
        }

        String channelName = getActualChannel(channelToSend, event.getChannel().getName());

        chat.sendMessage(channelName, STR."monakS \{ivrfi.getDisplayName()} is a Twitch staff NotLikeThis");
    }
}
