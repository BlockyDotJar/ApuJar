...
public class StartResumePlaybackCommand implements ICommand
{
    @Override
    public void onCommand(@NonNull ChannelMessageEvent event, @NonNull TwitchClient client, @NonNull String[] prefixedMessageParts, @NonNull String[] messageParts) throws Exception
    {
        ...
        {
            chat.sendMessage(event.getChannel().getName(), STR."AlienDance \{user.getDisplayName()} is already listening to a song.");
            return;
        }

        StartResumeUsersPlaybackRequest startResumePlaybackRequest = spotifyApi.startResumeUsersPlayback().build();
        startResumePlaybackRequest.execute();

        chat.sendMessage(event.getChannel().getName(), STR."jamm \{user.getDisplayName()} started/resumed his/her song.");
    }
}
